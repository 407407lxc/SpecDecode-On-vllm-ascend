...
def _get_backend_name(attn_backend: Any) -> str:
    get_name = getattr(attn_backend, "get_name", None)
...
def _backend_supports_spec_mqa(attn_backend: Any) -> Tuple[bool, str, str]:
    backend_name = _get_backend_name(attn_backend)

    supports_fn = getattr(attn_backend, "supports_spec_mqa_scorer", None)
    if callable(supports_fn):
        try:
            supports = bool(supports_fn())
            return supports, backend_name, "capability_fn"
        except Exception as exc:  # pylint: disable=broad-except
            logger.warning("supports_spec_mqa_scorer() failed: %s", exc)

    allowlist = _read_backend_allowlist_env(
        "VLLM_ASCEND_SPEC_MQA_BACKENDS", "FLASH_ATTN,ASCEND")
    supports = backend_name in allowlist
    return supports, backend_name, ",".join(sorted(allowlist))
...
def create_worker(
    ...
    if ngram_prompt_lookup_max > 0:
        ...
        if draft_model_config.hf_config.model_type == "mlp_speculator":
            ...
            if draft_tp == 1 and draft_model_config.hf_config.model_type !=\
                ...
                logger.info(
                    "[Speculative Decoding] Draft runner_cls=%s "
                    "(draft_tp=%s target_tp=%s model_type=%s)",
                    TP1DraftModelRunner,
                    draft_tp,
                    target_tp,
                ...
                logger.info(
                    "[Speculative Decoding] Draft runner_cls not set to TP1 "
                    "(draft_tp=%s target_tp=%s model_type=%s)",
                    draft_tp,
                    target_tp,
                    draft_model_config.hf_config.model_type,
    ...
    if not disable_mqa_scorer:
        attn_backend = scorer_worker.model_runner.attn_backend
        supports_mqa, backend_name, capability_source = \
            _backend_supports_spec_mqa(attn_backend)
        if not supports_mqa:
            disable_mqa_scorer = True
            logger.info(
                "[Speculative Decoding] Disabling MQA scorer as the backend "
                "capability is unavailable (backend=%s, capability=%s).",
                backend_name,
                capability_source,
            )
        else:
            logger.info(
                "[Speculative Decoding] MQA scorer backend capability "
                "detected (backend=%s, capability=%s).",
                backend_name,
                capability_source,
            )
