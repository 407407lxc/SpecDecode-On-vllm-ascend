...
class NPUWorker(LocalOrDistributedWorkerBase):
    ...
    def __init__(self,
        ...
        self._base_stage_log_enabled = os.getenv(
            "VLLM_ASCEND_BASE_STAGE_LOG_ENABLE", "0") == "1"
        self._base_stage_log_interval = int(
            os.getenv("VLLM_ASCEND_BASE_STAGE_LOG_INTERVAL", "200"))
        if self._base_stage_log_interval <= 0:
            self._base_stage_log_interval = 200
        self._base_stage_count = 0
        self._base_prepare_worker_ms = 0.0
        self._base_prepare_model_ms = 0.0
        self._base_execute_worker_ms = 0.0
        self._base_execute_model_ms = 0.0
        self._base_total_spmd_ms = 0.0
    ...
    def _execute_model_spmd(
        ...
        if not self._base_stage_log_enabled:
        ...
        output: Optional[List[SamplerOutput]]
        ...
        self._base_stage_count += 1
        self._base_prepare_worker_ms += (t1 - t0) * 1000
        self._base_prepare_model_ms += (t2 - t1) * 1000
        self._base_execute_worker_ms += (t3 - t2) * 1000
        self._base_execute_model_ms += (t4 - t3) * 1000
        self._base_total_spmd_ms += (t4 - t0) * 1000

        if self._base_stage_count % self._base_stage_log_interval == 0:
            n = self._base_stage_count
            logger.info(
                "BaseWorker stage times: avg_prepare_worker_ms=%.2f "
                "avg_prepare_model_ms=%.2f avg_cache_ops_ms=%.2f "
                "avg_execute_model_ms=%.2f avg_total_spmd_ms=%.2f "
                "count=%d",
                self._base_prepare_worker_ms / n,
                self._base_prepare_model_ms / n,
                self._base_execute_worker_ms / n,
                self._base_execute_model_ms / n,
                self._base_total_spmd_ms / n,
                n,
            )
