...
class NPUModelRunnerBase(ModelRunnerBase[TModelInputForNPU]):
    ...
    def __init__(
        ...
        self._runner_stage_log_enabled = os.getenv(
            "VLLM_ASCEND_RUNNER_STAGE_LOG_ENABLE", "0") == "1"
        self._runner_stage_log_interval = int(
            os.getenv("VLLM_ASCEND_RUNNER_STAGE_LOG_INTERVAL", "200"))
        if self._runner_stage_log_interval <= 0:
            self._runner_stage_log_interval = 200
        self._runner_stage_count = 0
        self._runner_forward_ms = 0.0
        self._runner_logits_ms = 0.0
        self._runner_sampler_ms = 0.0
        self._runner_total_ms = 0.0
    ...
    def load_model(self) -> None:
        ...
        if self.torchair_graph_enabled:
            ...
            # 闂備緡鍋呴惌顔界┍婵犲嫮涓嶆俊銈傚亾闁烩剝鐟╅獮瀣箛椤掍胶顩?            patch_for_hcom()
            # 闁荤姳绀佹晶浠嬫偪閸炲潮u闂佹眹鍔岄崹绫祅fig闂佹寧绋戦懟顖炪€呰瀵顭ㄩ埀顒傜箔婢跺本濯奸柛鎾楀懏鐎璫onfig闂佹寧绋戦懟顖濄亹閸欏顩烽柕澶堝€曢埢蹇涙煟椤剙濮傜紒顕呭灣閹峰濡堕崶銊ф殸闂佹寧绋戦惌鍌炲磻閸涙潙鐭楁い鏍ㄧ懁缁ㄤ即鎮规担绋库挃闁汇倕鏀硃u_backend="npu"
    ...
    def execute_model(
        ...
        **kwargs,
        ...
        runner_stage_enabled = (
            self._runner_stage_log_enabled and self.is_driver_worker
            and get_pp_group().is_last_rank)
        runner_total_start = time.perf_counter() if runner_stage_enabled else 0.0
        runner_forward_ms = 0.0
        runner_logits_ms = 0.0
        runner_sampler_ms = 0.0
        ...
        if not bypass_model_exec:
            forward_start = time.perf_counter() if runner_stage_enabled else 0.0
            with set_ascend_forward_context(
                ...
                hidden_or_intermediate_states = model_executable(
                    ...
                    **seqlen_agnostic_kwargs,
                    **model_kwargs)
            if runner_stage_enabled:
                runner_forward_ms = (time.perf_counter() - forward_start) * 1000
        ...
        logits_start = time.perf_counter() if runner_stage_enabled else 0.0
        logits = self.model.compute_logits(hidden_or_intermediate_states,
                                           model_input.sampling_metadata)
        if runner_stage_enabled:
            runner_logits_ms = (time.perf_counter() - logits_start) * 1000
        ...
        if self.is_driver_worker:
            ...
            sampler_start = time.perf_counter() if runner_stage_enabled else 0.0
            output: SamplerOutput = self.sampler(
                logits=logits,
                sampling_metadata=model_input.sampling_metadata,
            )
            if runner_stage_enabled:
                runner_sampler_ms = (time.perf_counter() - sampler_start) * 1000
            if (self.observability_config is not None
                    and self.observability_config.collect_model_forward_time
        ...
        if runner_stage_enabled:
            total_ms = (time.perf_counter() - runner_total_start) * 1000
            self._runner_stage_count += 1
            self._runner_forward_ms += runner_forward_ms
            self._runner_logits_ms += runner_logits_ms
            self._runner_sampler_ms += runner_sampler_ms
            self._runner_total_ms += total_ms
            if self._runner_stage_count % self._runner_stage_log_interval == 0:
                n = self._runner_stage_count
                logger.info(
                    "BaseRunner stage times: avg_forward_ms=%.2f "
                    "avg_logits_ms=%.2f avg_sampler_ms=%.2f "
                    "avg_execute_total_ms=%.2f count=%d",
                    self._runner_forward_ms / n,
                    self._runner_logits_ms / n,
                    self._runner_sampler_ms / n,
                    self._runner_total_ms / n,
                    n,
                )
