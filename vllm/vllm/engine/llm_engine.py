...
class LLMEngine:
    ...
    def _create_sequence_group_with_sampling(
        ...
        if self.vllm_config.speculative_config is not None:
            init_k = self.vllm_config.speculative_config.num_speculative_tokens
            # Pre-allocate acceptance stats for adaptive-k upper bound.
            adaptive_k_max = int(os.getenv("VLLM_ASCEND_ADAPTIVE_K_MAX",
                                           str(init_k)))
            draft_size = max(init_k, adaptive_k_max) + 1
        # ————————————————————————————
