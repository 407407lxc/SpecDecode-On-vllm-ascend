...
class AsyncMetricsCollector:
    ...
    def __init__(self,
        ...
        # __init__ 里加这三个状态
        self._min_k_observed: Optional[int] = None
        self._last_k_observed: Optional[int] = None
        self._dynamic_k_seen: bool = False
    ...
    def maybe_collect_rejsample_metrics(
        ...
        # Track dynamic-k state.
        if k > 0:
            if self._last_k_observed is not None and k != self._last_k_observed:
                self._dynamic_k_seen = True
            self._last_k_observed = k
            if self._min_k_observed is None:
                self._min_k_observed = k
            else:
                self._min_k_observed = min(self._min_k_observed, k)
    ...
    def _collect_rejsample_metrics(
        ...
        # _collect_rejsample_metrics 里替换这一段
        max_num_emitted_tokens = self.get_max_num_emitted_tokens(
            draft_tokens,
            k,
            self._min_k_observed,
            self._dynamic_k_seen,
        )
    ...
    def get_max_num_emitted_tokens(draft_tokens: int,
        ...
        """Fixed-k: exact. Dynamic-k: conservative upper bound."""
        if draft_tokens <= 0:
            return 0
