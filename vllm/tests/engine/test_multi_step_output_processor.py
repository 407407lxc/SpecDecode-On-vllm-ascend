...
@pytest.mark.skip_global_cleanup
def test_spec_acceptance_counts_expand_for_dynamic_k():
    """Dynamic-k can emit larger step_index than initial draft_size."""
    detokenizer = MagicMock(spec=Detokenizer)
    scheduler = MagicMock(spec=Scheduler)
    stop_checker = MagicMock(spec=StopChecker)
    seq_counter = Counter()
    ...
    # Simulate request-level counters allocated by a smaller initial k.
    seq_group.metrics.spec_token_acceptance_counts = [0, 0]
    ...
    counts = seq_group.metrics.spec_token_acceptance_counts
